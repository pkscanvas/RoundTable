...
class App(ctk.CTk):  # Inherit from ctk.CTk for the main application window
    def __init__(self):
        ...
        # Variable to store the index usage state
        self.use_index = tk.BooleanVar(value=False)  # Default to False
        self.create_widgets()
    ...
    def create_widgets(self):
        ...
        self.table_text_display = ctk.CTkTextbox(self.table_frame, state=tk.NORMAL, height=10)
        ...
        # Add the switch before query_input in the bottom_frame
        self.index_switch = ctk.CTkSwitch(bottom_frame, text="Use Index", variable=self.use_index, onvalue=True, offvalue=False)
        self.index_switch.pack(side=tk.LEFT, padx=(0, 5))

        self.query_input = ctk.CTkEntry(bottom_frame)
        self.query_input.pack(side=tk.LEFT, expand=True, fill=tk.X, padx=(0, 5))
        self.query_input.insert(0, "Enter your question here...")
    ...
    #         pt = Table(self.table_frame, dataframe=self.df, showtoolbar=False, showstatusbar=False ) #.head(10)
    #         pt.show()
    #         self.result_display_2.configure(state=tk.NORMAL)
    ...
    def display_dataframe(self):
        ...
        if self.df is not None:
            ...
            for index, row in self.df.iterrows():  # Adjust row display limit as needed .head(10)
    ...
    def on_submit(self):
        self.result_display_1.configure(state=tk.NORMAL)
        self.result_display_1.delete('1.0', tk.END)  # Clear previous output
        try:
            user_query = self.query_input.get()
            file_name = self.file_name_input.get()
            use_index = self.use_index.get()  # Get the current state of the switch
            max_attempts = 2
            attempt = 0
            ...
            while attempt < max_attempts:
                ...
                if error_occurred:
                    ...
                    self.result_display_2.configure(state=tk.NORMAL)
                    self.result_display_2.delete("1.0", tk.END)
                    self.result_display_2.insert(tk.END, f"Generated Code:\n{extracted_code}\n")  # Display extracted code in the app
